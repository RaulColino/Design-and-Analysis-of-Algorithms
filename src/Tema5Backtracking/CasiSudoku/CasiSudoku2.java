...
public class CasiSudoku2 {
    ...
    public static void main(String[] args) {
        ...
        comprobarSudoku(sudoku);
    }

    private static void comprobarSudoku(int[][] sudoku) {
        int numSoluciones = 0;
        CasiSudoku2 s = new CasiSudoku2();
        numSoluciones = s.resolver(sudoku, 0);
        if (numSoluciones == 0) {
    ...
    // devuelve 0 si no hay sol, 1 si hay una sol, 2 si hay varias
    private int resolver(int[][] sudoku, int numSol) {
        for (int f = 0; f < 9; f++) {
            for (int c = 0; c < 9; c++) {
                if (sudoku[f][c] == 0) {
                    for (int n = 1; n < 10; n++) {
